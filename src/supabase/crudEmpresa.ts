...
import type { Empresa, InsertarEmpresaParams, EditarEmpresaParams, MostrarEmpresaXIdUsuarioParams } from "../types";
...
export async function MostrarEmpresaXidsuario(p: MostrarEmpresaXIdUsuarioParams): Promise<any> {
...
export async function EditarEmpresa(p: EditarEmpresaParams, fileold: string | File, filenew: string | File): Promise<void> {
  const { error } = await supabase.from(tabla).update(p).eq("id", p.id);
  if (error) {
    throw new Error(error.message);
  }
...
async function subirImagen(idempresa: number, file: File): Promise<{ publicUrl: string }> {
