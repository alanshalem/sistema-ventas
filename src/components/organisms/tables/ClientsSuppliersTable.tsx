...
interface ClientsSuppliersTableProps {
  readonly data: ClientSupplier[]
  readonly setOpenRegister: (open: boolean) => void
  readonly setSelectedData: (data: ClientSupplier) => void
  readonly setAction: (action: string) => void
}

export function ClientsSuppliersTable({
  data,
  setOpenRegister,
  setSelectedData,
  setAction,
}: Readonly<ClientsSuppliersTableProps>) {
  if (data == null) return null

  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([])
  const { eliminarCliPro } = useClientesProveedoresStore()

  function handleDelete(item: ClientSupplier) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        await eliminarCliPro({ id: item.id })
      }
    })
  }

  function handleEdit(item: ClientSupplier) {
    if (item.nombre === 'General') {
      Swal.fire({
        icon: 'error',
        title: 'Oops...',
  ...
  const columns: ColumnDef<ClientSupplier>[] = [
    {
      accessorKey: 'nombres',
      header: 'Descripcion',
