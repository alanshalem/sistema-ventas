...
interface CategoriesTableProps {
  ...
  readonly setSelectedData: (data: Categoria) => void
...
export function CategoriesTable({
  ...
  function handleDelete(item: Categoria) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        await eliminarCategoria({ id: item.id })
      }
    })
  }
  ...
  const columns: ColumnDef<Categoria>[] = [
    ...
    },
    {
      ...
      cell: (info) => (
        <td data-title="Color" className="ContentCell">
          <ColorContent $color={info.getValue() as string} $height="25px" $width="25px" />
        </td>
      ),
      enableColumnFilter: true,
    },
...
const ColorContent = styled.div<{ $color: string; $height: string; $width: string }>`
  justify-content: center;
