...
import type { Caja, Sucursal } from '../../../types/database'
import { ButtonDashed } from '../../ui/buttons/ButtonDashed'

interface SucursalWithCajas extends Sucursal {
  caja: (Caja & { delete?: boolean })[]
  delete?: boolean
}

export function BranchesList(): ReactElement {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => mostrarCajasXSucursal({ id_empresa: dataempresa?.id ?? 0 }),
    enabled: !!dataempresa,
  })

  const editarSucursal = (p: SucursalWithCajas) => {
    const { caja, delete: _delete, ...sucursalData } = p
    selectSucursal(sucursalData)
  ...
  const editarCaja = (p: Caja & { delete?: boolean }) => {
    const { delete: _delete, ...cajaData } = p
    setStateCaja(true)
    setAccionCaja('Editar')
    setCajaSelectItem(cajaData)
  }

  const controladorEliminarCaja = (id: number) => {
  ...
  return (
    <Container>
      {data?.map((sucursal, index) => {
        const sucursalWithDelete = sucursal as SucursalWithCajas
        return (
          <Sucursal key={index}>
            <SucursalHeader>
              <Acciones $right="0px" $top="0px">
                {sucursalWithDelete?.delete && (
                  <Icon
                    icon="wpf:delete"
                    width="15"
                    height="15"
                ...
                <Icon
                  ...
                  width="20"
                  height="20"
                  onClick={() => editarSucursal(sucursalWithDelete)}
                />
              </Acciones>
              <SucursalTitle>SUCURSAL: {sucursal.nombre}</SucursalTitle>
            </SucursalHeader>
            <CajaList>
              {sucursalWithDelete.caja?.map((caja, index) => {
                return (
                  <CajaItem key={index}>
                    <CajaInfo>
