...
export function LowStockReport(): ReactElement {
  ...
  const { data, isLoading, error } = useQuery<StockBajoMinimoItem[] | null>({
    queryKey: [
  ...
  const renderTableRow = (rowData: StockBajoMinimoItem) => (
    <View style={styles.row} key={rowData.codigo_articulo}>
      ...
      <Text style={styles.numberCell}>
        {typeof rowData.stock === 'number' ? rowData.stock.toFixed(2) : rowData.stock}
      </Text>
      <Text style={styles.numberCell}>
        {typeof rowData.stock_minimo === 'number'
      ...
      <Text style={styles.numberCell}>
        {typeof rowData.total === 'number' ? rowData.total.toFixed(2) : rowData.total ?? '-'}
      </Text>
    </View>
  )

  const totalStock = data?.reduce((acc, item) => acc + (item.stock || 0), 0) || 0
  const totalValor = data?.reduce((acc, item) => acc + (item.total || 0), 0) || 0

  const currentDate = new Date()
  const formattedDate = `${currentDate.toLocaleDateString()} ${currentDate.toLocaleTimeString()}`

  const sampleData: StockBajoMinimoItem[] = [
    {
