...
export function SalesReport(): ReactElement {
  ...
  const { data, isLoading, error } = useQuery<VentaReportItem[] | null>({
    queryKey: [
    ...
    queryFn: async () => {
      const result = await reportVentasPorSucursal({
        id_empresa: sucursalesItemSelect?.id_empresa ?? 0,
        fecha_inicio: fechaInicio ?? undefined,
        fecha_fin: fechaFin ?? undefined,
      })
      return result as VentaReportItem[] | null
    },
  ...
  const cw = (p: number) => ({ flexBasis: `${p}%`, flexGrow: 0, flexShrink: 0 })
            ...
            <View style={styles.table}>
              ...
              {rows.length > 0 && (
                <View style={styles.totalRow}>
                  ...
                  <Text style={[styles.td, cw(COL.total), styles.right, styles.totalText]}>
                    {n(totalVentas)}
                  </Text>
                  ...
                  <Text style={[styles.td, cw(COL.cant), styles.center, styles.totalText]}>
                    {totalProductos}
                  </Text>
