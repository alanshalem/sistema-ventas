...
interface EditarCantidadParams {
  id: number
  cantidad: number
}

export function POSSaleDetailArea() {
  ...
  const EditarCantidadDv = async (data: EditarCantidadParams): Promise<void> => {
    const p = {
      id: data.id,
  ...
  const EliminarDV = async (p: DetalleVenta): Promise<void> => {
  ...
  const handleEditClick = (index: number, cantidad: number): void => {
    setEditIndex(index)
  ...
  const handleInputBlur = (item: DetalleVenta): void => {
  ...
  return (
    <AreaDetalleventa className={items && items.length > 0 ? '' : 'animacion'}>
            ...
            <Itemventa key={index}>
              ...
              <article className="contentbtn">
                ...
                <Button
                  ...
                  width="20px"
                  height="35px"
                  icon={<Icon icon="subway:subtraction-1" />}
                />
              </article>
...
const Itemventa = styled.section`
  ...
  justify-content: space-between;
  width: 100%;
  border-bottom: 1px dashed ${({ theme }) => theme.neutral};
