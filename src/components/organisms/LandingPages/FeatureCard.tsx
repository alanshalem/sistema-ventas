...
interface FeatureCardProps {
  ...
  readonly bgcontentimagen: string
...
export function FeatureCard({
  ...
  bgcontentimagen,
  subtitle,
}: Readonly<FeatureCardProps>): ReactElement {
  return (
...
const CardIcon = styled.div<{ $bgcontentimagen: string }>`
  width: 50px;
  height: 50px;
  border-radius: 50%;
...
const Badge = styled.span<{ $bgcontentimagen: string }>`
  position: absolute;
  top: 10px;
  right: 10px;
  background-color: ${(props) => props.$bgcontentimagen};
